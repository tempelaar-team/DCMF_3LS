...
@jit(nopython=True)
def Q_F_scaled(wavefn, lambda_alpha_grid):
    force = 2 * wgrid * nu12 * lambda_alpha_grid * np.real(np.conjugate(wavefn[0]) * wavefn[1])* np.abs(wavefn[1])**2
    force = force + 2 * wgrid * nu23 * lambda_alpha_grid * np.real(np.conjugate(wavefn[1]) * wavefn[2])*np.abs(wavefn[2])**2
    return force
...
def H_q(Q, Qzp, wavefn, lambda_alpha):
    ...
    for n in range(num_trj):
        intQ = (Q[:,n] + np.abs(wavefn[1,n]) ** 2 * Qzp[:,n])
        intQ2 = (Q[:,n] + np.abs(wavefn[2,n]) ** 2 * Qzp[:,n])
        Hq[1, 0, n] = np.sum(nu12 * w * lambda_alpha * intQ)
        Hq[2, 1, n] = np.sum(nu23 * w * lambda_alpha * intQ2)
    Hq[0, 1, :] = Hq[1, 0, :]
    Hq[1, 2, :] = Hq[2, 1, :]
    return Hq
...
def init_classical_parallel(wavefn, lambda_al, num_trj, proc):
    ...
    # need to add something to split whole Nxn_trj to N x Ntrj/proc x proc
...
def run_dyn(index, ini_wavefn, Q, P, QaQb, Qzp, Pzp, QaQbzp, lambda_al):
    ...
    for t in steps:
        ...
        for n in np.arange(0, num_atom):
            ...
            QFzp = QFzp + Q_F_scaled(now_wavefn, lambda_al_grid[n,:,:])
